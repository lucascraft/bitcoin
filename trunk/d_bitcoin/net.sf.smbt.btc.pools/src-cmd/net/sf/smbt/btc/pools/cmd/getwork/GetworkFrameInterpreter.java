...
public class GetworkFrameInterpreter  extends AbstractFrameInterpreterImpl {
	@Override
	public synchronized Cmd byteArray2Cmd(final byte[] stream) {
		GetworkCmd atCmd = GetworkFactory.eINSTANCE.createGetworkCmd();
		String json = "";
		if (stream != null) {
			json = new String(stream);
		}
		atCmd.setJsonRcv(json);
		return atCmd;
	}
}
