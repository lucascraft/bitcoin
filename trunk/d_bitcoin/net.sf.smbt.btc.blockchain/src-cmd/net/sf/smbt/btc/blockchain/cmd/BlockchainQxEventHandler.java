...
public class BlockchainQxEventHandler extends DefaultTxEventHandlerImpl {
	@Override
	public void handleQxEvent(Event evt) {
		if (evt.getCmd() instanceof BlockchainCmd) {
			...
			if (evt.getKind() == EVENT_KIND.TX_DONE) {
				...
				if (obj instanceof BlockchainHttpCommunicator) { // Http 
					((BlockchainHttpCommunicator) obj).get(evt);
				}
			}
		}
	}
