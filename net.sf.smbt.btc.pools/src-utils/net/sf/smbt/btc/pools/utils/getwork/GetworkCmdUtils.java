...
public class GetworkCmdUtils {
	
	public final static GetworkCmdUtils INSTANCE = new GetworkCmdUtils();
	

	public GetworkCmd handleGetworkFrame(byte[] frame) {
		GetworkCmd cmd = GetworkFactory.eINSTANCE.createGetworkCmd();
		String rcv = "";
		if (frame != null) {
			rcv = new String(frame);
		}
		cmd.setJsonRcv(rcv);
		return cmd;
	}
