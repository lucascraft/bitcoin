...
public class BitcoinRpcFrameInterpreter  extends AbstractFrameInterpreterImpl {
	@Override
	public synchronized Cmd byteArray2Cmd(final byte[] stream) {
		BitcoinCmd mtGoxCmd = BitcoindFactory.eINSTANCE.createBitcoinCmd();
		if (stream != null) {
			String text = new String(stream);
			mtGoxCmd.setJsonRcv(text);
		}
		return mtGoxCmd;
	}
}
